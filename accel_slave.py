...
class AccelerometerLogger():
    ...
    def send_accel(self, client):
        ...
        try:
            ...
            print('Failed to send accel values')
...
def receive_data(connection):
    data = connection.recv(12)  # Expecting to receive 12 bytes for 3 floats
    if data:
...
#host = '192.168.168.65'  # Replace with the server's IP address
...
if __name__ == '__main__':
    ...
    try:
        connection, server_socket = start_server(host, port)
    except Exception as e:
    ...
    try:
        ...
        folder_name = receive_string(connection)
        filename = receive_string(connection)
        
    except Exception as e:
        print(f"Error receiving folder name or filename: {e}")
        connection.close()
        server_socket.close()
    ...
    try:
        print('Init logger')
        sensor = AccelerometerLogger(filename, save_every, client, tag_X, desired_hz=desired_hz, R=R)
        sensor.calibrate()
